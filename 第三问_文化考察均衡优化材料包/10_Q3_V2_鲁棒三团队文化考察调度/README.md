...
- 基础现场项目天数：13 天；
- 仿真样本：15000 条；
- 成本敏感均衡策略：5 天项目缓冲，加权成功率 0.719，最差情景成功率 0.140；
- 高可靠执行策略：机动小组 + 5 天缓冲，加权成功率 0.817；
- 复合极端预案：延后/拆期 + 7 天缓冲，加权成功率 0.956。

该最优性结果是在楼兰古城和尼雅遗址固定由专项审批组执行、三组均乌鲁木齐起讫、资源不跨组共享的政策空间内成立。
...
> 在“楼兰古城、尼雅遗址固定为同一专项审批组，三组均从乌鲁木齐起讫，组内路径用高德/增强 OD 时间，资源不跨组共享”的政策口径下，枚举分配 + Held-Karp 得到精确最优。
