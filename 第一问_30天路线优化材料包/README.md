...
- `运营鲁棒主推版`：24 景点、25 个活动/转场日、5 天缓冲，运营成功率满足阈值，时间窗违规为 0；适合作为运营鲁棒口径下的现实主推，但不是完全舒适主推；
- `严格舒适主推备选`：20 景点、9 天缓冲，严格舒适成功率约 0.432；未达到 80%，只能作为保守备选，并提示需要继续降红色压力日；
- `极限覆盖版`：32 景点超过 30 天小时级可执行性边界，且仿真成功率为 0，仅作为覆盖上界；
- `亲子舒适版`：20 景点、9 天缓冲，成功率约 0.860；严格 0 红日方案未找到，因此采用独立亲子效用函数在放宽红日约束后选择；
- `长者慢游版`：20 景点、9 天缓冲，成功率约 0.850；严格 0 红日方案未找到，因此采用独立长者效用函数在放宽红日约束后选择。

论文中建议将 V2 的 30 景点方案作为“日级稳健候选”，将 V3 的 30 景点方案写成“小时级覆盖候选”，并将 V3 的 24 景点方案作为“运营鲁棒主推”。严格舒适口径必须单独展示，不应与运营成功率混用。
